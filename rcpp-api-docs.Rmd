...
**[Complex Operators](#sugar-complex)**
...
### Logical Operations {#sugar-logic}
...
- Convert the result state of `any( Y )` and `all( Y )` from the
...
- For example, if `any( Y )` evaluates to `false` than `is_true( X )` will return
  `false` but `is_false( X )` will return `true`.
