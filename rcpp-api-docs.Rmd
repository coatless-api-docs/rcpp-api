...
- **Warning:** Using any of the above member functions to grow or shrink 
  an Rcpp object will result in a severe degregation of performance. The 
  reason why is because Rcpp objects are proxy models that serve as shallow 
  wrappers around R objects.  Thus, when an Rcpp object is expanded or shrunk,
  the data must be _copied_ from the original object into a new object. 
  Therefore, it is highly recommended to convert the Rcpp object to an STL
  object that can easily be grown or shrunk if the sample size is _not_ known 
  in advance.
...
- Rcpp provides support for static and dynamic error messages.
  - Static error messages rely upon only a `string` being passed into the function.
  - Dynamic error messages must have a `string` with  `printf`-like format 
    identifiers (see table for a list of common specifiers) followed by an 
    appropriate number of parameters (e.g `param1`, `param2`, ...).
    - **Note:** Under C++98, there is a limit of 10 parameters for constructing
      a dynamic error message whereas C++11 or greater lacks this limitation.

- Error messages are sent over standard error (`STDERR`) stream
  instead of standard input (`STDIN`) stream like `Rcout<<`. This is beneficial
  as scripts that are not run interactively (e.g. over a distributed computing
  node) will typically report to a different log directory customized to
  warn the developer of an error regarding their job.
    
- Common `printf` identifiers

| specifier	   | Input                          |   Example   |
|:------------:|:------------------------------:|:-----------:|
| `%i` or `%d` | Signed integer	                | -42, 10     |
| `%u`         | Unsigned integer	            | 10          |
| `%f`         | Decimal floating point         | 3.14        |
| `%e`         | Scientific notation, lowercase	| 1.5379e+6   |
| `%E`         | Scientific notation, uppercase	| 1.5379E+6   |
| `%c`         | Character	                    | x           |
| `%s`         | String	                        | James  	  |
...
- End the present execution of _C++_ code, return control to _R_ session,
  and print an error message to the _R_ console.

```{Rcpp stop_ex}
// Static stop message
stop("Uh oh, we ran into an error.");

// Dynamic stop message
int index = 100;
std::string vec_name = "toad";
stop("Whoops, the index %i is bad at %s", index, vec_name);
```

#### `warning(...)` {#warning}

- Triggers a warning message to be displayed in the _R_ console while **not**
  aborting the _C++_ execution.


```{Rcpp stop_ex}
// Static warning
warning("Hey, was that supposed to do that?");

// Dynamic warning
std::string vec_name = "toad";
warning("Houston, I think we have a problem at %s. Do you copy?", vec_name);
```
...
- Caveat: at the time of writing (Rcpp version 0.12.11), not all Sugar expressions 
...
- Calculates the cumulative maximum (`cummax`) or cumulative minimum (`cummin`) 
  of a `Vector` or `Matrix` *X*. 
...
- Obtain a random sampling of elements from either a positive number of elements
  ranging from 1 to _n_ or data contained with _X_.
...
- Obtain the first locations of a match between elements in _X_ and _Y_.
...
- **Note:** This operation is similar to the _R_ command `match(x, unique(x))`.
...
- Determine whether elements in _X_ are found in _Y_.
...
#### trimws( X, which ) {#trimws}

- Trim leading and/or trailing whitespace from strings.

- The `which` argument controls how the trimming is performed. 
  This is **required** and can be specified with either
  `"l"` (leading), `"r"` (trailing), or `"b"` (both).

- Only the `Character` and `String` types of a `Vector` or `Matrix` are
  supported.
  
- This function provides support for returning either a `String`, `Vector`, or
  `Matrix` depending on the supplied parameter.
  
- Definition: **Whitespace** in the context of this function is considered to
  be either: space (` `), horizontal tab (`\t`), line feed (`\n`), or
  carriage return (`\r`).
