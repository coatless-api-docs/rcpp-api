...
| [`Arg`](#complex-argument)    | Arg (theta)                        |
...
|                       |                               |
|:----------------------|:------------------------------|
| [`head`](#prev-data)  | View the first _n_ values     |
| [`tail`](#prev-data)  | View the last _n_ values      |
...
- The `DataFrame` data structure is a typedef of the `DataFrame_Impl` class,
  ...
  [policy-based design based](https://en.wikipedia.org/wiki/Policy-based_design), 
...
- Supported types to perform the operation are `Integer` or `Numeric` of 
  a `Vector` or `Matrix`. 
...
- Compute the square root of all elements in a `Vector` or `Matrix`.

- Example:

```{Rcpp}
// Sample data
NumericVector x = NumericVector::create(0.0, 1.0, 2.5, 5.0, 9.0);
...
- Calculate the overall summation of all elements in a `Vector` or `Matrix`. 

- Definition: $$ \sum\limits_{i = 1}^n {{X_i}} $$
...
- Supported types are `integer` and `numeric` of a `Vector` or `Matrix`.
...
- Supported types are `Numeric`, `Logical`, `Complex`, `Raw`, `Character`, 
  `String`, and `Generic` of the `Matrix` class.
